...
var hello = async (event) => {
  ...
  if (Object.keys(user).length === 0) {
    if (event.body.firstName === void 0 || event.body.lastName === void 0) {
      ...
      const result = await dynamo.put({
        TableName: process.env.userTable,
        Item: __spreadProps(__spreadValues({}, event.body), {
          id: randomString(100, "abcdefghigjklmnopqrstuvwxyzAbcdefgh12364839847#$^&(*(&%$#`")
        })
      }).promise();
      return formatJSONResponse({
